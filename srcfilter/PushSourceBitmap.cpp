        ...
        m_rtFrameLength(FPS_30) // Display 30 bitmap frames per second
{
  /* // The main point of this sample is to demonstrate how to take a DIB
    ...
    HANDLE m_hFile = CreateFile(szFileCurrent, GENERIC_READ, 0, NULL, OPEN_EXISTING, 
   ...
   BYTE *m_pFile = new BYTE[dwFileSize];
    ...
    DWORD m_cbBitmapInfo = pBm->bfOffBits - cbFileHeader;
...
HRESULT CPushPinBitmap::GetMediaType(CMediaType *pMediaType)
{
    ...
    VIDEOINFOHEADER *pvi = (VIDEOINFOHEADER*)pMediaType->AllocFormatBuffer(SIZE_PREHEADER + sizeof(BITMAPINFOHEADER));
    if (pvi == 0) 
    ...
    memcpy(&(pvi->bmiHeader), &m_bmi, sizeof(BITMAPINFOHEADER));
...
HRESULT CPushPinBitmap::FillBuffer(IMediaSample *pSample)
{
	...
	// memset(pData, 0, min(pVih->bmiHeader.biSizeImage, (DWORD) cbData));
...
HRESULT CPushPinBitmap::SetVideoSize (SIZE size)
{
	m_bmi.bmiHeader.biWidth = size.cx;
	m_bmi.bmiHeader.biHeight = size.cy;

	m_bmi.bmiHeader.biSizeImage = m_bmi.bmiHeader.biWidth * m_bmi.bmiHeader.biHeight * m_bmi.bmiHeader.biBitCount/8;
	if (m_pImage)
		delete m_pImage;
	m_pImage = new BYTE[m_bmi.bmiHeader.biSizeImage ];
	return S_OK;
}

HRESULT CPushPinBitmap::SetRawData (BYTE *data)
{
	memcpy(m_pImage, data, m_bmi.bmiHeader.biSizeImage);
	return S_OK;
}
...
CPushSourceBitmap::CPushSourceBitmap(IUnknown *pUnk, HRESULT *phr)
           : CSource(NAME("VideoSourceFilter"), pUnk, CLSID_VideoSourceFilter)
{
...
{
	if (riid == IID_IVideoSourceFilter)
	{
		return GetInterface(static_cast<IVideoSourceFilter*>(this), ppv);
	}
	else
	{
...
STDMETHODIMP CPushSourceBitmap::SetVideoSize (SIZE size)
{
	return m_pPin->SetVideoSize(size);
}

STDMETHODIMP CPushSourceBitmap::SetRawData (BYTE *data)
{
	return m_pPin->SetRawData(data);
}
//-----------------------------------------------------------------------------
